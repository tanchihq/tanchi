...
const AssistantStream = ({ actions, text }: AssistantStreamProps) => {
  ...
  return (
    <div className="flex justify-start gap-2.5">
      ...
      <div className="max-w-[80%] rounded-2xl border border-app-line bg-app-surface px-[15px] py-2.5 text-[13.5px] leading-relaxed text-app-fg">
        {actions.length > 0 && (
          <div className={cn('flex flex-col gap-1', done && 'mb-2')}>
            {actions.map((name, index) => (
              <div
                key={`${name}-${index}`}
                className="flex items-center gap-1.5 text-[12px] leading-none text-app-faint"
              >
                {done ? (
                  <Check size={12} className="text-app-success-fg shrink-0" />
                ) : (
